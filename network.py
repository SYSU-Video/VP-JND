...
def distorted_inputs(filenames):
    ...
    """
    patches_x, patches_y, labels = MCL_JCI_input.distorted_inputs(filenames=filenames, batch_size=BATCH_SIZE)
...
def inputs(filenames):
    ...
    Raises:
        :exception ValueError: If no data_dir
    """
    patches_x, patches_y, labels = MCL_JCI_input.inputs(filenames=filenames, batch_size=BATCH_SIZE)
...
def inference(patches_x, patches_y, keep_prob):
    ...
    with tf.variable_scope('fuse_features'):
        feature_x = tf.reshape(tensor=feature_map_x, shape=[MCL_JCI_input.NUM_PATCHES_PER_IMAGE * BATCH_SIZE, -1])
        feature_y = tf.reshape(tensor=feature_map_y, shape=[MCL_JCI_input.NUM_PATCHES_PER_IMAGE * BATCH_SIZE, -1])
    ...
    with tf.variable_scope('weighting') as scope:
        ...
        patches_weights = tf.reshape(patches_weights, (BATCH_SIZE, MCL_JCI_input.NUM_PATCHES_PER_IMAGE)) + EPSILON
    ...
    with tf.variable_scope('estimate_quality') as scope:
        patches_qualities = tf.reshape(patches_qualities, (BATCH_SIZE, MCL_JCI_input.NUM_PATCHES_PER_IMAGE))
...
def loss_func(scores, labels):
    ...
    with tf.variable_scope('Loss'):
        ...
        loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.cast(labels, tf.float32), logits=scores))
